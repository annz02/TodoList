use std::sync::Arc;

use anyhow::Result;
use reqwest::header::{ACCEPT, USER_AGENT};
use semver::Version;
use serde::{Deserialize, Serialize};
use tauri::{AppHandle, Emitter, State};
use tauri_plugin_updater::UpdaterExt;

// ── Constants ────────────────────────────────────────────────────────────

const GITHUB_RELEASES_API_URL: &str =
    "https://api.github.com/repos/annz02/TodoList-Ann/releases?per_page=100";
const GITHUB_RELEASES_ACCEPT: &str = "application/vnd.github+json";
const GITHUB_RELEASES_USER_AGENT: &str = "todolist-updater";
const EMPTY_RELEASE_NOTES_TEXT: &str = "No release notes were provided for this version.";

pub(crate) const UPDATE_STATUS_EVENT: &str = "update-status";

// ── State types ───────────────────────────────────────────────────────────

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct UpdateStatus {
    pub(crate) status: String,
    pub(crate) current_version: String,
    pub(crate) latest_version: Option<String>,
    pub(crate) body: Option<String>,
    pub(crate) published_at: Option<String>,
    pub(crate) downloaded_bytes: Option<u64>,
    pub(crate) content_length: Option<u64>,
    pub(crate) error: Option<String>,
}

impl UpdateStatus {
    fn idle(current_version: String) -> Self {
        Self {
            status: "idle".into(),
            current_version,
            latest_version: None,
            body: None,
            published_at: None,
            downloaded_bytes: None,
            content_length: None,
            error: None,
        }
    }
}

#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct UpdateDebugStatePayload {
    pub(crate) status: String,
    pub(crate) latest_version: Option<String>,
    pub(crate) body: Option<String>,
    pub(crate) published_at: Option<String>,
    pub(crate) downloaded_bytes: Option<u64>,
    pub(crate) content_length: Option<u64>,
    pub(crate) error: Option<String>,
}

pub(crate) struct UpdateState {
    pub(crate) status: std::sync::Mutex<UpdateStatus>,
    pub(crate) pending: std::sync::Mutex<Option<tauri_plugin_updater::Update>>,
    pub(crate) debug_override: std::sync::Mutex<Option<UpdateStatus>>,
}

impl UpdateState {
    pub(crate) fn new(current_version: String) -> Self {
        Self {
            status: std::sync::Mutex::new(UpdateStatus::idle(current_version)),
            pending: std::sync::Mutex::new(None),
            debug_override: std::sync::Mutex::new(None),
        }
    }
}

// ── Internal helpers ──────────────────────────────────────────────────────

#[derive(Clone, Copy)]
enum CheckSource {
    Startup,
    Manual,
    Recovery,
}

#[derive(Debug, Clone, Deserialize)]
struct GithubRelease {
    tag_name: String,
    body: Option<String>,
    draft: bool,
}

fn emit_status(app: &AppHandle, state: &Arc<UpdateState>, next: UpdateStatus) -> UpdateStatus {
    *state.status.lock().unwrap() = next.clone();
    let _ = app.emit(UPDATE_STATUS_EVENT, &next);
    next
}

fn version_of(app: &AppHandle) -> String {
    app.package_info().version.to_string()
}

fn parse_release_version(raw: &str) -> Option<Version> {
    Version::parse(raw.trim().trim_start_matches('v')).ok()
}

fn release_notes_section(version: &str, body: Option<&str>) -> String {
    let content = body
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(EMPTY_RELEASE_NOTES_TEXT);
    format!("## {version}\n\n{content}")
}

fn build_aggregated_release_notes(
    current_version: &str,
    latest_version: &str,
    latest_body: Option<&str>,
    releases: &[GithubRelease],
) -> Option<String> {
    let current = parse_release_version(current_version)?;
    let latest = parse_release_version(latest_version)?;
    let mut matched = releases
        .iter()
        .filter(|release| !release.draft)
        .filter_map(|release| {
            let version = parse_release_version(&release.tag_name)?;
            if version <= current || version > latest {
                return None;
            }
            Some((version, release))
        })
        .collect::<Vec<_>>();

    matched.sort_by(|left, right| right.0.cmp(&left.0));

    if matched.is_empty() {
        return latest_body
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(|value| release_notes_section(latest_version, Some(value)));
    }

    let notes = matched
        .into_iter()
        .map(|(_, release)| release_notes_section(&release.tag_name, release.body.as_deref()))
        .collect::<Vec<_>>()
        .join("\n\n---\n\n");

    Some(notes)
}

async fn fetch_aggregated_release_notes(
    current_version: &str,
    latest_version: &str,
    latest_body: Option<&str>,
) -> Result<Option<String>> {
    let client = reqwest::Client::new();
    let releases = client
        .get(GITHUB_RELEASES_API_URL)
        .header(ACCEPT, GITHUB_RELEASES_ACCEPT)
        .header(USER_AGENT, GITHUB_RELEASES_USER_AGENT)
        .send()
        .await?
        .error_for_status()?
        .json::<Vec<GithubRelease>>()
        .await?;

    Ok(build_aggregated_release_notes(
        current_version,
        latest_version,
        latest_body,
        &releases,
    ))
}

// ── Debug override ────────────────────────────────────────────────────────

fn active_debug_status(state: &Arc<UpdateState>) -> Option<UpdateStatus> {
    if !cfg!(debug_assertions) {
        return None;
    }
    state.debug_override.lock().unwrap().clone()
}

fn build_debug_status(
    app: &AppHandle,
    payload: UpdateDebugStatePayload,
) -> Result<UpdateStatus, String> {
    let status = payload.status.trim().to_string();
    if !matches!(
        status.as_str(),
        "idle" | "checking" | "available" | "downloading" | "downloaded" | "up_to_date" | "error"
    ) {
        return Err("invalid_update_debug_status".into());
    }

    Ok(UpdateStatus {
        status,
        current_version: version_of(app),
        latest_version: payload.latest_version,
        body: payload.body,
        published_at: payload.published_at,
        downloaded_bytes: payload.downloaded_bytes,
        content_length: payload.content_length,
        error: payload.error,
    })
}

// ── Public API ────────────────────────────────────────────────────────────

pub(crate) fn spawn_startup_check(app: AppHandle, state: Arc<UpdateState>) {
    tauri::async_runtime::spawn(async move {
        let _ = check_for_updates_inner(app, state, CheckSource::Startup, false).await;
    });
}

#[allow(dead_code)]
pub(crate) fn spawn_manual_check(app: AppHandle, state: Arc<UpdateState>) {
    tauri::async_runtime::spawn(async move {
        let _ = check_for_updates_inner(app, state, CheckSource::Manual, false).await;
    });
}

async fn check_for_updates_inner(
    app: AppHandle,
    state: Arc<UpdateState>,
    source: CheckSource,
    force: bool,
) -> Result<UpdateStatus> {
    // Debug override takes priority
    if let Some(next) = active_debug_status(&state) {
        *state.pending.lock().unwrap() = None;
        return Ok(emit_status(&app, &state, next));
    }

    // Idempotent guard
    let current = state.status.lock().unwrap().clone();
    if !force && matches!(current.status.as_str(), "checking" | "downloading") {
        return Ok(current);
    }

    emit_status(
        &app,
        &state,
        UpdateStatus {
            status: "checking".into(),
            current_version: version_of(&app),
            latest_version: None,
            body: None,
            published_at: None,
            downloaded_bytes: None,
            content_length: None,
            error: None,
        },
    );

    let result = app.updater()?.check().await;

    // Re-check debug override (might have been set during check)
    if let Some(next) = active_debug_status(&state) {
        *state.pending.lock().unwrap() = None;
        return Ok(emit_status(&app, &state, next));
    }

    match result {
        Ok(Some(update)) => {
            let aggregated_body = match fetch_aggregated_release_notes(
                &version_of(&app),
                &update.version,
                update.body.as_deref(),
            )
            .await
            {
                Ok(body) => body.or_else(|| update.body.clone()),
                Err(error) => {
                    eprintln!("update release notes fetch error: {error}");
                    update.body.clone()
                }
            };
            let next = UpdateStatus {
                status: "available".into(),
                current_version: version_of(&app),
                latest_version: Some(update.version.clone()),
                body: aggregated_body,
                published_at: update.date.as_ref().map(ToString::to_string),
                downloaded_bytes: None,
                content_length: None,
                error: None,
            };
            *state.pending.lock().unwrap() = Some(update);
            Ok(emit_status(&app, &state, next))
        }
        Ok(None) => {
            *state.pending.lock().unwrap() = None;
            Ok(emit_status(
                &app,
                &state,
                UpdateStatus {
                    status: "up_to_date".into(),
                    current_version: version_of(&app),
                    latest_version: None,
                    body: None,
                    published_at: None,
                    downloaded_bytes: None,
                    content_length: None,
                    error: None,
                },
            ))
        }
        Err(error) => {
            *state.pending.lock().unwrap() = None;
            // Startup check: silent fallback to idle, don't bother the user
            if matches!(source, CheckSource::Startup) {
                return Ok(emit_status(
                    &app,
                    &state,
                    UpdateStatus::idle(version_of(&app)),
                ));
            }
            Ok(emit_status(
                &app,
                &state,
                UpdateStatus {
                    status: "error".into(),
                    current_version: version_of(&app),
                    latest_version: None,
                    body: None,
                    published_at: None,
                    downloaded_bytes: None,
                    content_length: None,
                    error: Some(error.to_string()),
                },
            ))
        }
    }
}

// ── Tauri commands ────────────────────────────────────────────────────────

#[tauri::command]
pub(crate) fn get_update_state(
    state: State<'_, Arc<UpdateState>>,
) -> Result<UpdateStatus, String> {
    Ok(state.status.lock().unwrap().clone())
}

#[tauri::command]
pub(crate) async fn check_for_updates(
    app: AppHandle,
    state: State<'_, Arc<UpdateState>>,
) -> Result<UpdateStatus, String> {
    check_for_updates_inner(app, state.inner().clone(), CheckSource::Manual, false)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub(crate) async fn install_update(
    app: AppHandle,
    state: State<'_, Arc<UpdateState>>,
) -> Result<UpdateStatus, String> {
    let upd_state = state.inner().clone();

    // Debug mode: pretend download completed
    if cfg!(debug_assertions) && active_debug_status(&upd_state).is_some() {
        let current = upd_state.status.lock().unwrap().clone();
        let downloaded_bytes = current
            .downloaded_bytes
            .or(current.content_length)
            .or(Some(100));
        let downloaded_status = UpdateStatus {
            status: "downloaded".into(),
            current_version: version_of(&app),
            latest_version: current.latest_version,
            body: current.body,
            published_at: current.published_at,
            downloaded_bytes,
            content_length: current.content_length.or(downloaded_bytes),
            error: None,
        };
        return Ok(emit_status(&app, &upd_state, downloaded_status));
    }

    let pending = upd_state
        .pending
        .lock()
        .unwrap()
        .take()
        .ok_or_else(|| "No update is ready to install".to_string())?;

    let mut next = upd_state.status.lock().unwrap().clone();
    if next.latest_version.is_none() {
        next.latest_version = Some(pending.version.clone());
    }
    next.status = "downloading".into();
    next.error = None;
    next.downloaded_bytes = Some(0);
    next.content_length = None;
    emit_status(&app, &upd_state, next.clone());

    let app_for_progress = app.clone();
    let state_for_progress = upd_state.clone();
    let latest_version = pending.version.clone();
    let published_at = pending.date.as_ref().map(ToString::to_string);
    let body = pending.body.clone();
    let mut downloaded = 0u64;

    if let Err(error) = pending
        .download_and_install(
            move |chunk_length, content_length| {
                downloaded += chunk_length as u64;
                let progress = UpdateStatus {
                    status: "downloading".into(),
                    current_version: version_of(&app_for_progress),
                    latest_version: Some(latest_version.clone()),
                    body: body.clone(),
                    published_at: published_at.clone(),
                    downloaded_bytes: Some(downloaded),
                    content_length: content_length.map(|value| value as u64),
                    error: None,
                };
                emit_status(&app_for_progress, &state_for_progress, progress);
            },
            || {},
        )
        .await
    {
        eprintln!("update install error: {error}");
        return check_for_updates_inner(app, upd_state, CheckSource::Recovery, true)
            .await
            .map_err(|e| e.to_string());
    }

    let downloaded_status = UpdateStatus {
        status: "downloaded".into(),
        current_version: version_of(&app),
        latest_version: next.latest_version,
        body: next.body,
        published_at: next.published_at,
        downloaded_bytes: next.downloaded_bytes,
        content_length: next.content_length,
        error: None,
    };

    // Windows: installer needs UAC, let user restart manually.
    // macOS / Linux: auto restart after download.
    #[cfg(windows)]
    {
        return Ok(emit_status(&app, &upd_state, downloaded_status));
    }
    #[cfg(not(windows))]
    {
        emit_status(&app, &upd_state, downloaded_status);
        app.restart();
    }
}

// Debug-only: inject fake update UI states without hitting real APIs.
#[tauri::command]
pub(crate) fn set_update_debug_state(
    app: AppHandle,
    state: State<'_, Arc<UpdateState>>,
    payload: Option<UpdateDebugStatePayload>,
) -> Result<UpdateStatus, String> {
    if !cfg!(debug_assertions) {
        return Err("update_debug_unavailable".into());
    }

    let upd_state = state.inner().clone();
    *upd_state.pending.lock().unwrap() = None;

    let next = match payload {
        Some(payload) => {
            let next = build_debug_status(&app, payload)?;
            *upd_state.debug_override.lock().unwrap() = Some(next.clone());
            next
        }
        None => {
            *upd_state.debug_override.lock().unwrap() = None;
            UpdateStatus::idle(version_of(&app))
        }
    };

    Ok(emit_status(&app, &upd_state, next))
}

#[cfg(test)]
mod tests {
    use super::{build_aggregated_release_notes, GithubRelease};

    #[test]
    fn aggregates_release_notes_between_current_and_latest() {
        let releases = vec![
            GithubRelease {
                tag_name: "v0.1.26".into(),
                body: Some("Latest patch".into()),
                draft: false,
            },
            GithubRelease {
                tag_name: "v0.1.25".into(),
                body: Some("Current version".into()),
                draft: false,
            },
        ];

        let notes =
            build_aggregated_release_notes("0.1.24", "0.1.26", None, &releases)
                .expect("aggregated notes");

        assert!(notes.contains("## v0.1.26"));
        assert!(notes.contains("## v0.1.25"));
    }

    #[test]
    fn skips_draft_releases() {
        let releases = vec![
            GithubRelease {
                tag_name: "v0.1.26".into(),
                body: Some("Draft release".into()),
                draft: true,
            },
            GithubRelease {
                tag_name: "v0.1.25".into(),
                body: Some("Released version".into()),
                draft: false,
            },
        ];

        let notes =
            build_aggregated_release_notes("0.1.24", "0.1.26", None, &releases)
                .expect("aggregated notes");

        assert!(notes.contains("## v0.1.25"));
        assert!(!notes.contains("## v0.1.26"));
    }

    #[test]
    fn falls_back_to_latest_body_when_no_matching_releases() {
        let releases = vec![GithubRelease {
            tag_name: "v0.1.23".into(),
            body: Some("Older version".into()),
            draft: false,
        }];

        let notes = build_aggregated_release_notes(
            "0.1.24",
            "0.1.26",
            Some("Latest release notes"),
            &releases,
        )
        .expect("fallback notes");

        assert!(notes.contains("## 0.1.26"));
        assert!(notes.contains("Latest release notes"));
    }
}
